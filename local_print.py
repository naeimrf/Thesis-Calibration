...
def GetData(path):
    ...
    for i, key in enumerate(ResBld):
        Res['elec'].append(
            ResBld[key]['EnergyConsVal'][1] / 3.6 / ResBld[key]['EPlusHeatArea'] * 1000)  # convert GJ in kWh/m2
        ...
        Res['tot'].append((ResBld[key]['EnergyConsVal'][1] + ResBld[key]['EnergyConsVal'][4] +
                           ResBld[key]['EnergyConsVal'][5]) / 3.6 / ResBld[key][
                              'EPlusHeatArea'] * 1000)  # to have GJ into kWh/m2
        eleval = 0
        ...
        try:
            TotalPower[key] = [ResBld[key]['HeatedArea']['Data_Zone Ideal Loads Supply Air Total Cooling Rate'][i] +
                               ...
                               for i in range(
                    len(ResBld[key]['HeatedArea']['Data_Zone Ideal Loads Supply Air Total Heating Rate']))]
...
def GetSingleSim(path, buildList=[]):
    ...
    print('organizing data...')
    if len(ResBld) > 1:
        print('Sorry, it seems that there are at least 2 simulation results file in this path...')
    else:
        Res = {}
...
def SignaturePlots(data):
    ...
    for i, key in enumerate(data):
        ...
        ax0.plot(data[key]['OutdoorSite']['Data_Site Outdoor Air Drybulb Temperature'],
                 data[key]['HeatedArea']['Data_Zone Ideal Loads Supply Air Total Heating Rate'], '.')
        ax0.plot(data[key]['OutdoorSite']['Data_Site Outdoor Air Drybulb Temperature'],
...
def DistAnalyse(mainPath):
    ...
    for i in range(5, 28):
        path = mainPath + str(
            i) + '\\Sim_Results\\'
        Res = GetData(path)
        plotRelRes(NewFig['fig_name'].number, NewFig['ax0'], Res['Dist'], Res['heat1'], 'Shading Distance (m)',
                   'Building_' + str(i))
        ...
        path2idf = mainPath + str(
            i) + '\\'
        print('For Building_' + str(i) + ' : Mini is sim_' + str(Res['heat1'][minDist]) + ' / Maxi is sim_' + str(
            Res['heat1'][maxDist]))
        print('For Building_' + str(i) + ' : increasing dist leads to:' + str(Res['tot1'][minDist] -
                                                                              Res['tot1'][
                                                                                  maxDist]) + 'of kWh/m2 of differences')
...
def wwrLeakAnalyse(path):
    Res = GetData(path[0])
    NewFig = createSimpleFig()
    plotResBase(NewFig['fig_name'].number, NewFig['ax0'], Res['EnvLeak'], Res['PowerPic'], 'Env Leak (l/s/m2 for 50Pa)',
                'PowerPic', '', '.')
    ...
    for key in Data:
        if 'TempExt' not in key:
            if len(np.shape(Data[key])) == 1:
                NewSet[key] = [val for i, val in enumerate(Data[key]) if
                               Data[Var][i] <= range[1] and Data[Var][i] >= range[0]]
            else:
        ...
        for key in Res:
            ...
            for key1 in Res[key]:
                if 'Data_' in key1:
                    ...
                    plotResBase(NewFig[key][key1]['fig_name'].number, NewFig[key][key1]['ax0'],
                                [i for i in range(0, len(Res[key][key1]))], Res[key][key1], 'Time', key1, key, '-')
        ...
        for key in Res:
            ...
            for key1 in Res[key]:
                if 'Data_' in key1:
                    ...
                    plotResBase(currentfig['fig_name'].number, currentfig['ax0'],
                                [i for i in range(0, len(Res[key][key1]))], Res[key][key1], 'Time', DataLabel, Unit)
...
def InertiaAnalyses(path, keyword):
    for i, current_path in enumerate(path):
        ...
        for k in range(len(Res1['Powjr'][1, :, 1])):
            distrib = [(Res1['Powjr'][26, k, j] - np.mean(Res1['Powjr'][26, k, :])) for j in
                       range(len(Res1['Powjr'][26, k, :]))]
...
if __name__ == '__main__':
    ...
    # some useful functions are implemented such as aggregating the data from several simulation in one dictionary
    # this makes it easy the plot of variables over others.
    # some generic functions to create figure and make plots are also available.
    # but specific functions are to be created for specific analyses. some are left as example but they might not be
    # compatible with other simulations than the one they were created for!
    # some propose plot for time-series with over plots of other specific cases while other function enable to plot not time dependent variables
    ...
    # available functions
    # wwrLeakAnalyse(path)
    DynAnalyse(path, ['Building_10v0.pickle', 'Building_10v1.pickle'])
    # GlobRes(path[0])
